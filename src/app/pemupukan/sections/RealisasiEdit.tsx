...
export default function RealisasiEditPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        const res = await fetch(`/api/pemupukan/realisasi?id=${idParam}`, {
          cache: "no-store",
        });
        if (!res.ok) {
          const text = await res.text();
          console.error("GET detail error:", text);
          throw new Error(text || "Gagal mengambil data realisasi");
        }
        ...
        Swal.fire({
          ...
          text: "Terjadi kesalahan saat mengambil data realisasi.",
        }).then(() => {
          router.back();
        });
      } finally {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      await Swal.fire({
        icon: "success",
        title: "Berhasil",
        text: "Data realisasi pemupukan berhasil diperbarui.",
      });
  ...
  return (
    <section className="space-y-2">
      <SectionHeader
        title="Edit Realisasi Pemupukan"
        desc="Perbarui data realisasi sesuai laporan lapangan."
      />
