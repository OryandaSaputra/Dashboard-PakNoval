...
export async function GET() {
  ...
  const rows = plans.map(p=>{
    ...
    return {
      kebun: p.kebun, kodeKebun: p.kode_kebun, tanggal: r?.tanggal ?? "",
      afd: p.afd, tt: String(p.tt), blok: p.blok, jenisPupuk: p.jenis_pupuk,
      rencanaKg: p.kg_pupuk, realisasiKg: r?.kg_pupuk ?? 0, stokKg: totalStokByKebun(p.kebun),
    };
  });
